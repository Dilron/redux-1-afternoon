...
import store, {UPDATEINSTRUCTION, UPDATERECIPES} from '../../store'
...
class Instructions extends Component {
  ...
  componentDidMount(){
    store.subscribe(() => {
      ...
      this.setState({instructions: tempState.instructions})
    })
  }
  handleChange(val) {
    this.setState({
  ...
  addInstruction() {
    store.dispatch({type: UPDATEINSTRUCTION, payload: this.state.input})
    this.setState({
      input: ""
    });
  }
  create() {
    store.dispatch({type: UPDATERECIPES})
  }
  render() {
